...
import net.succ.succsessentials_extended.block.custom.RollingMillBlock;
...
public class RollingMillBlockEntity extends AbstractPoweredMachineBlockEntity
    ...
    public final ItemStackHandler itemHandler = new ItemStackHandler(2) {
        ...
        public boolean isItemValid(int slot, ItemStack stack) {
            return slot == INPUT && stack.is(ModTags.Items.INGOTS);
        }
    ...
    protected boolean hasRecipe() {
        Optional<RecipeHolder<RollingMillRecipe>> recipe = getCurrentRecipe();
        if (recipe.isEmpty()) return false;

        RollingMillRecipe value = recipe.get().value();
    ...
    private Optional<RecipeHolder<RollingMillRecipe>> getCurrentRecipe() {
        return level.getRecipeManager().getRecipeFor(
                ModRecipes.ROLLING_MILL_TYPE.get(),
                new RollingMillRecipeInput(itemHandler.getStackInSlot(INPUT)),
                level
        );
    }
    ...
    protected void setLitState(boolean lit) {
        ...
        if (state.getValue(RollingMillBlock.LIT) != lit) {
            level.setBlock(worldPosition, state.setValue(RollingMillBlock.LIT, lit), 3);
        }
    }
    ...
    public Component getDisplayName() {
        return Component.translatable("block.succsessentials_extended.rolling_mill");
    }

    @Nullable
    @Override
    public AbstractContainerMenu createMenu(int id, Inventory inv, Player player) {
        return new RollingMillBlockMenu(id, inv, this, data);
    }
